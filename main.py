...
def main():
    if len(sys.argv) < 5:
        print("Usage: python main.py '<bias_config>' '<groups_config>' '<disease_config>' '<correlated_config>'")
        sys.exit(1)

    # Parse the JSON strings for bias configuration, groups configuration, disease configuration, and correlated variables configuration from command-line arguments
    bias_config = json.loads(sys.argv[1])
    groups_config = json.loads(sys.argv[2])
    disease_config = json.loads(sys.argv[3])
    correlated_config = json.loads(sys.argv[4])
    ...
    simulator = HealthDataSimulator(num_samples=1000, feature_details=feature_details, bias_config=bias_config, groups_config=groups_config, disease_config=disease_config, correlated_config=correlated_config)
